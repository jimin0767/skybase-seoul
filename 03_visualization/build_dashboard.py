...
<div class="dashboard">
  ...
  <div class="main-grid">
    <div class="map-panel">
      <div id="map"></div>
      <div id="bldg-status" style="position:absolute;bottom:32px;left:10px;background:rgba(10,10,26,0.82);color:#7ebfe8;padding:4px 10px;border-radius:6px;font-size:11px;font-family:'Noto Sans KR',sans-serif;z-index:10;pointer-events:none;border:1px solid #0f3460;"></div>
    </div>
...
map.on('load', () => {{
  ...
  // ── 1. Hide CARTO flat 2D building fills ─────────────────────
  // CARTO dark-matter has 'building' and 'building-top' flat fill layers.
  // They show as gray rectangles and visually mask the 3D extrusion layer below.
  ['building', 'building-top'].forEach(id => {{
    if (map.getLayer(id)) map.setLayoutProperty(id, 'visibility', 'none');
  }});

  // ── 2. Terrain (set FIRST — must precede any fill-extrusion layer) ────
  // setTerrain() reconfigures the 3D rendering pipeline; fill-extrusion
  // layers added afterwards sit correctly on the terrain surface.
  ...
  // ── 3. H3 source + circle layer (anchor for beforeId ordering) ──
  ...
  // Fallback: CARTO vector tile buildings (no file load required, always visible).
  // Seoul OSM has no height attrs → fixed 20 m default so blocks appear immediately.
  // Replaced by custom GeoJSON layer when that finishes loading.
  map.addLayer({{
    id: 'buildings-carto-3d',
    type: 'fill-extrusion',
    source: 'carto',
    'source-layer': 'building',
    minzoom: 10.5,
    paint: {{
      'fill-extrusion-color': '#3a6ea8',
  ...
  // Primary: custom GeoJSON with real heights (2.5× exaggeration for readability).
  // Starts hidden; set visible + hide fallback when fetch completes.
  ...
  map.addLayer({{
    ...
    paint: {{
      ...
      // 2.5× exaggeration so buildings read clearly in the tilted 3D view
      'fill-extrusion-height': ['*', ['to-number', ['get', 'height_m'], 0], 2.5],
      'fill-extrusion-base': 0,
  ...
  _bldgStatus.textContent = '🏢 3D 빌딩: 로딩 중…';
  fetch('./tableau_data/seoul_buildings_3d.geojson')
    ...
    .then(data => {{
      ...
      map.setLayoutProperty('buildings-carto-3d', 'visibility', 'none');
      _bldgStatus.textContent = `🏢 3D 빌딩: ${{data.features.length.toLocaleString()}}개 로드됨`;
      console.log('3D buildings loaded:', data.features.length, 'features');
    }})
    .catch(err => {{
      _bldgStatus.textContent = `⚠️ 빌딩 로드 실패 (CARTO 폴백 사용 중): ${{err.message}}`;
      console.warn('Buildings GeoJSON load failed, using CARTO fallback:', err);
    }});
